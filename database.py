...
class Database:
    ...
    def get_movies_by_category(self, category):
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            print(f"DEBUG: Kategoriya qidirilmoqda: {category}")  # DEBUG qo'shing
            ...
            ''', (f'%{category}%',))  # % belgilarini qo'shing
            results = cursor.fetchall()
            print(f"DEBUG: Topilgan kinolar soni: {len(results)}")  # DEBUG
            return results
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                SELECT movie_id, title, description, category, file_id, price, 
                       is_premium, actor_name, banner_file_id, created_at, added_by 
                FROM movies 
                WHERE category LIKE '%Maxsus%' OR category LIKE '%Exclusive%'
                ORDER BY created_at DESC
            ''')
            return cursor.fetchall()

    def add_user_exclusive_access(self, user_id, duration_days=30):
        """Foydalanuvchiga maxsus kontentlar uchun ruxsat berish"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            
            # Jadval mavjudligiga ishonch hosil qilish
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS user_exclusive_access (
                    user_id INTEGER PRIMARY KEY,
            ...
            access_expires_at = access_granted_at + timedelta(days=duration_days)
            
            cursor.execute('''
                INSERT OR REPLACE INTO user_exclusive_access 
                (user_id, access_granted_at, access_expires_at, is_active) 
                VALUES (?, ?, ?, ?)
            ''', (user_id, access_granted_at, access_expires_at, True))
            conn.commit()

    def check_exclusive_access(self, user_id):
        """Foydalanuvchining maxsus kontentlarga kirish huquqini tekshirish"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT access_expires_at FROM user_exclusive_access 
                WHERE user_id = ? AND is_active = TRUE
            ''', (user_id,))
            result = cursor.fetchone()
    ...
    def add_premium_subscription(self, user_id, duration_days=30, amount=130000):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                INSERT OR REPLACE INTO premium_subscriptions 
                (user_id, start_date, end_date, payment_amount, payment_status) 
                VALUES (?, ?, ?, ?, ?)
            ''', (user_id, start_date, end_date, amount, 'active'))
            conn.commit()
